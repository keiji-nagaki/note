<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="flex justify-between h-16">
      <div class="flex">
        ...
        <!-- 🔽 一覧ページへのリンクを追加 -->
        <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
          <x-nav-link :href="route('specification.index')" :active="request()->routeIs('specification.index')">
            {{ __('機器一覧') }}
          </x-nav-link>
        </div>
        <!-- 🔽 作成ページへのリンクを追加 -->
        <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
          <x-nav-link :href="route('specification.create')" :active="request()->routeIs('specification.create')">
            {{ __('仕様書登録') }}
          </x-nav-link>
        </div>
       <!-- 🔽 マイページへのリンクを追加 -->
  ...
  <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
    ...
    <!-- 🔽 一覧ページへのリンクを追加 -->
    <div class="pt-2 pb-3 space-y-1">
      <x-responsive-nav-link :href="route('specification.index')" :active="request()->routeIs('specification.index')">
        {{ __('機器一覧') }}
      </x-responsive-nav-link>
    </div>
    <!-- 🔽 作成ページへのリンクを追加 -->
    <div class="pt-2 pb-3 space-y-1">
      <x-responsive-nav-link :href="route('specification.create')" :active="request()->routeIs('specification.create')">
        {{ __('仕様書登録') }}
      </x-responsive-nav-link>
    </div>
    <!-- 🔽 マイページへのリンクを追加 -->
